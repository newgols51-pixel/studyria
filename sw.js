...
// ── VERSION ───────────────────────────────────────────────────────
const CACHE_VERSION = 'v158' // v158: NOTIFICATION DELIVERY SYSTEM V2 — permanent external-HTTPS destinations. (1) notificationclick now opens validated external HTTPS URLs EXACTLY as stored (WhatsApp/Telegram/YouTube/Instagram…) instead of rewriting them to the homepage; internal Studyria routes keep the existing focus-or-open behaviour. Same defence-in-depth rules as the admin validator: https scheme only, no javascript:/data:/file:/blob:, no embedded credentials, no control chars — an invalid URL honestly falls back to the site root. (2) Push payload gains CTA support (data.cta/payload.cta) for the action button, defaulting to 'Open' for external destinations; body-click always navigates identically (CTA never depends on action-button support). (3) Options gain timestamp + subtle vibrate [80,40,80] + explicit silent:false (OS-default sound — custom audio is NOT claimable on Web Push) + renotify:false (same-tag in-place update, never a duplicate sound). (4) The existing manual-notification banner resolver now also applies record.cta as the action label and corrects the click destination from the validated record.destination — same silent same-tag re-show, so a WhatsApp CTA works end-to-end even when the dispatch payload carries no cta field. No new service worker, no cache wipe, all existing caching/offline/push/sync handlers untouched.  // v157:  // v157: VFA PRACTICE BATCH 4 — 280 new NCERT/10+2 practice MCQs (Biology 163 · Chemistry 41 · Physics 76) in vfa-practice-pool-b4.js: diversity/classification, plant & animal kingdoms, animal tissues, sense organs, microbes in human welfare, redox/electrochemistry, pressure/buoyancy, magnetism/motors, oscillations, SONAR, lens power. §17 deduped (2 near-dupes replaced pre-deploy, 0 collisions final). Under existing subject approvals VFA grows 10→14/20 tests (1400 unique allocations, engine §12F validated 14/14; pools Bio 564 / Chem 314 / Phys 286 / GA 2324). Driver stays 6; fail-closed baseline 2. Remaining shortfall for Tests 15–20: Bio 236 / Chem 86 / Phys 114. vfa-practice-pool-b4.js?v=1.  // v156:  // v156: VFA PRACTICE BATCH 3 — 241 new NCERT/10+2 practice MCQs (Biology 160 · Chemistry 40 · Physics 41) in vfa-practice-pool-b3.js, AI-authored+disclosed, is_pyq=false, §17 deduped (17 near-dupes vs bank/batch-1/2 caught & replaced pre-deploy, 0 collisions final). Under existing subject approvals VFA grows 6→10/20 tests (1000 unique allocations across Tests 1–10, engine §12F validated, 0 reuse; pools Bio 401 / Chem 273 / Phys 210 / GA 2324). Fail-closed preserved; Driver stays 6. Remaining shortfall for Tests 11–20: Bio 399 / Chem 127 / Phys 190. vfa-practice-pool-b3.js?v=1.  // v155:  // v155: ZERO-SQL PUBLIC APPROVALS MIRROR — site_config has no public read policy yet and the owner can't run the one-time SQL now, so anonymous students read zero approval rows. New /brainlab-approvals.json (version-controlled, publicly served, synced from site_config by the agent on owner approve/revoke) is now the anon fallback source: dbInit tries site_config first (rows win — future-proof for when the SQL runs), falls back to the mirror fetch, fail-closed either way. Admin Test Engine shows a live mirror-sync status line (what students actually see + out-of-sync diff + last-updated). Mirror synced now with Biology (batch-1+2, 151 practice Qs) → anon students see VFA 3/20 tests (Bio 241→6, Chem 135→6, Phys 61→3 — binding: Physics). brainlab-tests.js?v=20260914h, brainlab-admin-tests.js?v=6, brainlab-approvals.json?v=1.  // v154: // v154: VFA PRACTICE BATCH 2 — 140 new NCERT/10+2-level practice MCQs (Biology 60 · Chemistry 40 · Physics 40) in vfa-practice-pool-b2.js, AI-authored+disclosed, is_pyq=false, needs owner approval per subject in Test Engine (fail-closed). QA: 0 collisions vs bank/batch-1 (§17 normQ hash — 6 near-dupes caught & replaced pre-deploy), with approvals VFA grows 4→6/20 tests (600 unique allocations, 0 reuse, all validated); without approvals stays 2. Shortfall after b2: Bio 559 / Chem 167 / Phys 231. vfa-practice-pool-b2.js?v=1.  // v153: // v153: VFA 20-TEST HONEST CAPACITY SYSTEM — §19/§27 user catalog now shows per-test honest status for unpublished tests: 'Test 5–20: Preparing — verified questions being added' with per-subject binding constraints computed from real engine counts (Biology 181/800 · Chemistry 193/400 · Physics 129/400 · GA surplus), never fake '100 MCQs' labels. §28 admin Pool Dashboard upgraded: per-subject Verified unique / Used / Unused / Required-for-all-target / Shortfall columns from live pool math. Full §30 QA: VFA 4/20 published (T1–T4 each exactly 40 Bio · 20 Chem · 20 Phys · 20 GA · 100 total, 0 within-test dups, 0 cross-test reuse across 400 unique allocations, 0 cross-exam leakage, all PASS); T5–20 honestly blocked by verified-content shortfall (Bio 619 · Chem 207 · Phys 271 needed) — NO fabrication per spec. brainlab-tests.js?v=20260914g, brainlab-admin-tests.js?v=5.  // v152: // v152: ZERO-MIGRATION APPROVALS — Test Engine no longer requires any SQL. Practice-set approvals now live in the EXISTING site_config table (key brainlab_practice_approvals; production RLS already gates writes to admin profiles), read by dbInit as a third job and written by the panel's Approve/Revoke buttons. The v3 migration (blueprints/registry/usage) is demoted to an optional advanced layer (collapsed details with copy buttons kept). Fail-closed preserved: no approval row = set locked; QA: zero-SQL+approved → VFA 4 tests / Driver 6 tests all validated; zero approvals → VFA 2 / Driver 0; cross-series leak check passed. brainlab-tests.js?v=20260914f, brainlab-admin-tests.js?v=4.  // v151: MIGRATION ACTIVATION FIX — root cause found: the v3 migration SQL had function EXECUTE grants but NO table grants; on Supabase, PostgREST (anon/authenticated) gets 42501 permission-denied on the new tables even with RLS read policies, so the Test Engine panel would say 'migration not active' even AFTER running the SQL. Fix: (a) grants block added to the migration file (SELECT on blueprints+registry to anon/authenticated, INSERT+SELECT on usage+versions, NOTIFY pgrst reload) + standalone sql/bl3-grants-fix.sql; (b) engine dbInit now classifies PGRST205=not_run vs 42501=no_grants (BT.DB.status); (c) Admin Test Engine gets an actionable Migration Helper — one-click copy buttons that fetch the 3 SQL files from the site into the clipboard + grants-fix button + 'I ran it — recheck' recheck. brainlab-tests.js?v=20260914e, brainlab-admin-tests.js?v=3.  // v150: ADRE DRIVER ROAD TRANSPORT PRACTICE SET — 138 practice MCQs (MV Act 1988, traffic signs, road markings, right-of-way, first aid, eco-driving, permits; all verifiable standard facts, deduped vs bank & VFA set, is_pyq=false, needs_review until owner approval; series-scoped registry 'adre-g3-driver' via sql/adre-driver-road-transport-seed.sql). Road Transport blueprint matcher wired — approving the set in Admin → Test Engine unlocks the verified ADRE 2.0 Driver Grade-III series (150Q/3h: SS20 GK30 LR20 Eng30 Math30 RT20). Admin review queue generalized: subject→series map (Bio/Chem/Phys→VFA, Road Transport→ADRE Driver). adre-road-transport-pool.js?v=1, vfa-practice-pool.js?v=2, brainlab-admin-tests.js?v=2, brainlab-tests.js?v=20260914d.  // v149: 5 NEW VERIFIED EXAM BLUEPRINTS (researched+corroborated): Assam Police SI (SLPRB: 100Q/100marks/3hrs/neg½ · LR-Aptitude-Comprehension 35, History&Culture of India&Assam 35, GK 30), ADRE 2.0 Grade-III HSSLC (150Q/180min/neg¼ · SS30 GK30 LR20 Eng35 Math35), ADRE Grade-IV HSLC & Class VIII (135Q/150min/neg¼ · SS30 GK30 Eng25 Math25 Reason25), ADRE Driver (150Q incl Road Transport 20 — honest Content Pending, no bank pool yet). DHS + MTS stay needs_verification honestly (no authoritative pattern confirmed). §17 dedup strengthened: canonical normQ(120)+answer key kills the 22 punctuation-variant near-duplicate bank pairs (1 cross-test dup in adre-g3 eliminated; all pools recompute honestly). brainlab-tests.js?v=20260914c; migration seed updated w/ verified rows + idempotent refresh UPDATEs.  // v148: BRAINLAB TESTS V3 — EXAM ECOSYSTEM GOVERNANCE: DB-driven layer (sql/brainlab-tests-v3-migration.sql — bl_test_blueprints / bl_question_registry / bl_question_usage / bl_test_versions + admin RPCs; verified DB rows OVERRIDE JS fallback, fail-open to v2 when not migrated). Registry governance: rejected/needs_review hashes NEVER enter public tests (absent = legacy bank). VFA practice set 1: 217 new practice MCQs (Bio 91/Chem 58/Phys 68, deduped vs bank, AI-authored+disclosed, is_pyq=false, all needs_review) in vfa-practice-pool.js — enter the VFA pool ONLY after owner approves in the new Admin → 🧪 Test Engine tab (§24 pool manager, §25 blueprint manager, §26 pool health, §23/§36 validation QA + leakage runner). Test snapshots + usage recorded server-side on open (spec §20/§28). Honest VFA pool: Bio 90/Chem 135/Phys 61/GA 2320 → 2 full tests; practice approval grows it. brainlab-tests.js?v=20260914b, brainlab-admin-tests.js?v=1, vfa-practice-pool.js?v=1.  // v147: BRAINLAB TESTS — EXAM TEST BLUEPRINT SYSTEM (content-correctness fix). Root cause: every series drew from one shared generic pool (Assam GK/GK/English/Maths/Reasoning/Science) with pool-arithmetic sizes, so e.g. VFA tests contained random Maths/English/Reasoning questions and no series followed its real exam pattern. Now: BT.BLUEPRINTS (verified official configs w/ sources) — VFA 2026: 100 Q/120 min/Biology 40·Chemistry 20·Physics 20·GA 20; Assam Police Constable AB/UB (SLPRB): 100 Q/50 marks, 5 sections × 20; APSC CCE Prelims GS-I (apsc.nic.in): 100 Q/120 min/200 marks/neg 0.5, open official-subject pool; SSC GD (official): 80 Q/60 min/4×20/neg 0.25; Assam TET LP: 150 Q/150 min/5×30 (blocked honestly — no CDP/Language-I pool yet). Exam-scoped subject pools w/ hard isolation (one question → one section, never inferred from text), disjoint seeded slices across tests, §12F validation on every test (exact counts, membership, zero dup) — invalid = not published. Unverified exams (SI, MTS, ADRE ×4, DHS) → needs_verification → hidden from users, Admin shows 'Blueprint verification required'. durationMin threads to the attempt timer (VFA = 120 min). Stale mid-attempt resume states cleared once (finished history preserved). brainlab-tests.js v2 + brainlab-tests.css, brainlab.js, brainlab-mock.js, brainlab-testpage.js, campus-admin.js bumped. // v146: // v146: per user request the YT full-song bar is AUDIO-ONLY again — video NOT shown in the player bar (lighter layout, much friendlier to low-data mobile users); ⛶ PiP button hidden with it (PiP needs a visible video). Foreground = full licensed YT audio; background = Apple 30s preview chain, unchanged. zubeen-da.css?v=10, zubeen-da.js?v=9. // v145: // v145: player-bar layout restructure — video now top-left, full control row (title/buttons/progress/hint) stacked BELOW it at full bar width instead of squeezed beside it. Also removed a leftover duplicate/stale mobile media-query block for .yt-live sizing that was winning the cascade over the v144 fix (118x66/10px old values instead of the intended tighter ones) — confirms v144's browser-side verification used inline JS overrides that bypassed this bug; this rewrite consolidates to one clean rule set. zubeen-da.css?v=10. // v144: // v144: player-bar mobile fix pt.2 — .z-player-info had flex-basis:0 with no shrink share, so under real overflow it collapsed to literally 0 width (title could vanish too), not just truncate. Gave buttons a firm mobile floor + info flex-basis:auto/min-width:44px so it always keeps real room. zubeen-da.css?v=10. // v143: // v143: mobile player-bar overflow fix — srcmode hint text (⛶ background tip) was garbling/spilling off-screen on narrow phones when a full YT song plays; shortened copy + nowrap/ellipsis truncation + tighter mobile sizing for yt-live row. zubeen-da.js?v=9, zubeen-da.css?v=10. // v142: // v142: batch-4 data — 3 newly verified songs published (Xopun Xopun from his final film Roi Roi Binale, DIU NIDIU KOI, Bauli Dhun — all Assamese, verified by lyric transcription; Prem Baru & Lekar Hum Diwana Dil Remix withheld as unverifiable; Baho Mein Aaja Snam withheld — rip watermark confirmed). Archive 309→312. zubeen-songs.js?v=5. // v141: // v141: one-line cleanup — _vis visible branch also removes the 'native' class when resuming the same full YT song (cosmetic; .apple-mode already removed so no CSS impact, kept for class hygiene). zubeen-da.js?v=9. // v140: // v140: Zubeen Da FULL-SONG BACKGROUND — visible mini video + Picture-in-Picture. Google blocks full-song autoplay of YT embeds in background (Jan 2026), so the automatic 30s preview chain stays; but for the FULL song the YT embed is now VISIBLE in the player bar (152×86 mini video, 118×66 mobile, .yt-live) — a display:none iframe can never enter PiP. New ⛶ button (#z-player-pip) fullscreens the embed via requestFullscreen(); swiping home then lets Android Chrome offer Picture-in-Picture and the WHOLE song keeps playing (the standard free YouTube background path). Assamese srcmode hint explains the flow on every YT song; native apple-mode gets .native class so the leftover YT iframe stays hidden (fixes a visual bug from v138 where apple native mode showed the YT iframe via .apple-mode vid CSS). zubeen-da.js?v=9, zubeen-da.css?v=+1, allowfullscreen on the embed. // v139: // v139: Zubeen Da BACKGROUND PREVIEW MODE — YouTube embeds are suspended by Chrome/YouTube when the page is hidden (Google closed the silent-audio loophole, Jan 2026; keeper alone no longer suffices on current Chrome). Instead of silence: when a YT song is playing and the page goes hidden, the SAME song's licensed official 30s Apple preview starts on OUR native <audio> (real background playback), auto-chains to the next song's preview, OS notification honestly labels 'Background preview — Apple Music 30s'; on return, full YouTube playback resumes from the exact suspension point (or the chained song in full). apple_preview now attached to ALL 309 archive songs (not just Apple-sourced) + ZUBEEN_PREVIEW_FALLBACKS map for 5 curated songs (ya-ali, janu-janu, keuphale, jag-lal-lal, sopunore-pokhi) → 314/324 songs background-capable; the remaining 10 (album jukeboxes etc., no licensed preview) stay honestly foreground-only. zubeen-da.js?v=9 (visibilitychange handler _vis, _bgNext chain, _previewOf), zubeen-songs.js?v=5. // v138: Zubeen Da APPLE NATIVE AUDIO — all 105 Apple Music songs now play via OUR OWN native <audio> element (official 30s Apple preview file) instead of the embed iframe: REAL background playback on Android/desktop Chrome (first-party audio is never background-muted, unlike cross-origin YT/Apple iframes — that is also why the YT 204 remain honest-foreground), real progress bar + seek, OS media controls drive whichever player is live, auto-advance on preview end, embed iframe kept as fallback for songs without a preview. zubeen-songs.js gains apple_preview field (105 verified URLs); zubeen-da.js?v=9, zubeen-songs.js?v=5. // v137: Zubeen Da archive 292→309 — 17 new verified songs published (12 Hindi incl. Instant Karma remixes, Ya Ali Trending Version, Bas Ek Pal OST remix, unplugged covers, Rang De Chunariya, Ek Nazar; 4 Assamese: O Priya from Priyar Priyo, Ei Je Morom, Tumar Morome & Shurat Mogon from Hits of Jayanta Hazarika; 1 Bengali: Aaj Baaja Tui Dhaak all-star puja single) — every new track verified by Whisper transcription + album/film context; 8 more sketchy releases honestly withheld (unauthorized rips with audible watermarks: Baho Mein Aaja Snam (Russian subtitle credit), Tumi Suwa Jetiya Lofi Remix (Korean outro), Lekar Hum Diwana Dil Remix ('have a good one guys'); Open Stage cover by another singer; tribute medley; karaoke track; unverifiable previews) — withheld list now 14 with honest reasons. BACKGROUND PLAYBACK FIX: near-silent looping native audio keeper grants the page persistent audio focus so Chrome (Android/desktop) keeps the YouTube iframe playing in background with the existing Media Session notification; iOS excluded honestly (Safari suspends iframes regardless). New asset zubeen-silence.wav; zubeen-da.js?v=9, zubeen-songs.js?v=5. // v136: Zubeen Da archive 280→292 — 12 previously-withheld songs verified & published (5 Assamese: Bohagore, Nilim Aakahe, Rumaal, Rock Rock Rock, Rajani Dhuhkhor; 5 Hindi: Ishq Hua, Tu Meri Aashiqui, Allah Allah, Pal Pal, Ya Khuda; 2 Bengali: Jhar Othe Mone, Ajke Ei Khushir Dine from Bindaas 2014) — language verified via Whisper transcription of Apple Music previews + album/film context; withheld now 2 with honest reasons. // v135: Zubeen Da background playback — Media Session API (OS media controls: play/pause/next/prev/seek + lock-screen metadata & position, artwork = tribute poster), auto-advance queue when a song ends, mediaSession released on close/apple/fail. zubeen-da.js?v=9. // v134: EGRESS GUARD — cover images auto-compress before upload (WebP ≤800px, supabase.js helper studyCompressImage, 5 upload sites + cacheControl 604800); cover-compressor.html one-time maintenance tool for existing covers bucket files; supabase.js?v=20260912a, pdf-classification-refactor.js?v=20260912a // v133 // v133: Zubeen Da Legacy Admin Console tab (🕯️ TRIBUTE section) — honest content overview + RLS-gated moderation dashboard/queue; Library active category pill restored to maroon→gold gradient; text-visibility-fix.css?v=20260912b; // v132 // v132: Library visual migration to cream/maroon/gold completed — text-visibility-fix.css compat layer accents re-tokened (crimson/amber/purple/teal → Royal Maroon #930205 / Premium Gold #c99a3c / deep green #2f6f4f), blanket section-badge override removed (per-section semantic accents now apply), active category pill restored to maroon→gold gradient; library-expansion-ui.js smart-shelf accents re-tokened; My Library premium banner navy → maroon. text-visibility-fix.css?v=20260912a, library-expansion-ui.js?v=20260912a. // v131: tribute hero upgraded to the user's commissioned poster (Prasanta Kr Das) — mobile full-bleed cover (text bottom-anchored, face+gamocha unobstructed), desktop grid shows the COMPLETE artwork beside text; viewport-scoped hero assets (each device downloads only its own); secondary CTA '🕊️ স্মৃতিৰ যাত্ৰা'; v130: tribute page polish — concise nav (8 links, fits 1280px) + clearer tribute-submission notice; v129 // v129: Zubeen Da — The Legacy tribute page (/zubeen-da) + subtle homepage footer tribute link; v128 // v128: answer-integrity — toQuiz option shuffle now DETERMINISTIC (seeded per question text) so resumed/retried sessions keep saved answers aligned with the exact options picked; v127 v127: BrainLab sub-route deep links now open correctly on FIRST LOAD (#brainlab/tests, #brainlab/tests/<series>, attempt URLs, subjects/org/cycle routes) — navigate() invokes the V8 sub-router after BrainLab.init() instead of waiting for a hashchange that never fires; _appInit accepts brainlab/<sub> hashes immediately (no home flash). v126: BrainLab TESTS module (see v126 note below for full detail) // v126: BRAINLAB TESTS MODULE — dedicated first-class Tests section (additive): #brainlab/tests catalog (real live stats, working search, org filters, honest sorts) → Test Series page (#brainlab/tests/<series>) → DEDICATED full-screen attempt page (#brainlab/tests/<series>/<n>) via the SAME mock engine + test page system with activity_type mode='test' (timer, palette, resume, auto-submit, EN/অ language, review — all reused, no second engine). 12 real test series (VFA 21, Constable AB/UB 12, SI 12, MTS 16, ADRE G3/G4/Driver/VIII 10 each, APSC CCE, Assam TET, DHS, SSC GD) — every test is a deterministic DISJOINT seeded block from the real verified question pool; published counts computed live (min(target, floor(pool/perTest))), zero fake questions/counts, content-pending shown honestly. Admin BrainLab Manager gains Test Series Manager (real pools, allocation, per-test preview, gaps). brainlab-tests.js/.css NEW; pages/testpage/mock/campus-admin bumped v20260911a. Zero changes to existing modules, engine scoring, auth, payment, checkout. // v122: DEDICATED ACTIVITY PAGES — every question-based BrainLab module (Quiz / MCQ / PYQ / Subject / Daily / Current Affairs / Arena / Mistakes / Flashcards) now opens its OWN full-screen attempt page (brainlab-activity.js, additive): shared question engine reused as-is (renderer, answer state, scoring, v7 premium result, Assamese translation, session recording) — only the page shell is new (mode-driven registry: label + back destination per activity mode). Flashcards: honest card backs (the CORRECT option text + explanation — was option A regardless of answer), per-card reveal state, progress bar, full controls (← Previous / ↻ Again / ✓ Know It / Next →). Practice buttons flex-wrap at 320-430px (full labels, 44px tap height, no clipping). CA 30+ direct-mock sessions also get a dedicated page shell. // v121: v121: expired-mock auto-submit no longer uses a blocking alert() (froze the page + blocked automation); honest auto-submit happens silently and the result page IS the notice. // v120: v120: BRAINLAB FINAL UX / PAGE SEPARATION / ANSWER REVIEW CORRECTION. Mock controls stay ENGLISH in Assamese mode (question content only is translated — spec §5). FIX(mock scoring): seeded option permutation flagged isCorrect against a RANDOM option (text was keyed by label, src by perm) — identity mapping keeps the same visual order and scores correctly. FIX(learn player): selectedAnswer stored the boolean original instead of the option letter — reviews/mistake book showed 'Your answer: true'. Review page: Your Answer + Correct Answer now show the real option TEXT, honest attempted-based Accuracy, UNANSWERED + MARKED review filters, English chrome. ALL mocks (list cards, count-picker, retries) open the DEDICATED full-screen test page — result + review render ON the dedicated page (no mixed hub content); Retry reruns the same fixed set; Exit returns to Exam Hub / Mock Test list. Responsive mock controls: flex-wrap buttons, full labels, no ellipsis, 44px tap height, safe-area — 320px+. // v119: AUTOMATED ASSAMESE TRANSLATION + AUTO-VERIFICATION (no manual review required). euTranslate v2: every machine translation runs a 5-stage server-side validation pipeline (structural → number/formula/% preservation → script/language → semantic consistency → safety/sanitize) with up to 3 automatic attempts; passing rows persist as system_verified (displayed with NO badge — honest: never claimed official/human-verified), failures keep the best safe translation as auto_translated with the honest স্বয়ংক্ৰিয় badge. Deterministic auto-correction (HTML/script/control-char stripping) applies ONLY to the translation — English originals never modified. QuestionTranslation rows gain translationVersion/attempts/note fields; priority verified > reviewed > system_verified > auto_translated; failed rows never re-hit the provider after 3 attempts. ADMIN: ⚡ Auto Translate & Verify Assamese bulk runner (batches of 12 through the server pipeline — resumable localStorage cursor, rate-limited, idempotent skip-existing, progress bar, failure log; Phase 2 processes imported ExamQuestion rows via admin-gated euManage bulkTr) + trStats DB-count stats; no manual verification needed for normal operation. Mock badge map: system_verified/verified → no badge. Zero engine-scoring/auth/payment/checkout changes. // v118: BILINGUAL TRANSLATION FALLBACK // v118: BILINGUAL TRANSLATION FALLBACK (Assamese) — new additive brainlab-tr-server.js (window.BLTR): when a question has no verified Assamese content, the mock/test-page player now loads an Assamese translation from a persistent SERVER-side cache (new QuestionTranslation Base44 entity + euTranslate backend: public cache read 'get', server-side en→as generation via a public Google endpoint — no API keys/secrets anywhere, persisted status='auto', provenance tracked, existing rows NEVER overwritten so verified/reviewed always win). Question+options+explanation all translated; short symbol tokens (Sn/Ti/Fe/Zn/%/dates) preserved exactly. Per-question on-demand with a small 'অনুবাদ লোড হৈ আছে…' loading note + honest 'স্বয়ংক্ৰিয় অনুবাদ' badge for machine rows (never labelled verified/official); whole-test background prefetch in chunks (cancellable, never blocks; cached rows reused everywhere). English originals never modified; no duplicate question records (translation keyed by content hash); answers/timer/marks/index preserved across EN↔অ and during translation arrival (re-render swaps text only). Learn player's BrainLabTranslate.translateQuestionAsync now consults the server cache before the legacy client path. euManage gains admin trList/trSave/trDelete review ops (edit Assamese, mark Reviewed/Verified — verified takes priority). brainlab-mock.js→v20260910b / testpage→v20260910b / tr-server v20260910a. Zero engine-scoring/auth/payment/checkout changes. // v117: dedicated test page. // v117: DEDICATED TEST PAGE — Mock Test cards now open a SEPARATE full-screen test page (route #brainlab/exams/mock/<examId>/<n>); the Exam Hub stays a discovery page. NEW additive brainlab-testpage.js/.css (bl-tp- ns): own header (title + EN/অ language toggle), sticky countdown, scrollable body — the mock engine renders INTO the page (#bl-tp-area), no second engine. Attempt persistence: answers/marks/question-index/timer-endAt/language/start-time saved per exam+mock → refresh, Android back, accidental exit all RESUME the same attempt (deterministic question order + seeded option order keep resume aligned); timer is attempt-scoped (endAt persisted, no fresh timer after reload); expired-while-away = honest auto-submit; leaving asks confirmation and KEEPS the saved attempt. ASSAMESE FIX: mock player is now language-aware per question — verified Assamese content (question_as + opt_*_as from bank/imported questions) shows real Assamese question AND options; if Assamese is genuinely unavailable an honest notice shows instead of silently showing English; mid-test EN↔অ switch preserves answers/timer/marks/index — only the displayed content changes. Mock answers now store the real option letter for the review page + mistake book (scoring unchanged). Submit/auto-submit route through the ORIGINAL BrainLab._finishQuiz (session recording, streak, mistakes, review — untouched). Zero backend/DB/auth/payment/checkout changes; learn player and all other modes untouched. // v116: exam-specific mock naming. // v116: EXAM-SPECIFIC MOCK TEST NAMING — hub Mock Tests now lists '[Exam Name] — Mock Test 1..N' (AssamTest-style vertical list, Studyria identity, org icon + chevron, real 'N Minutes · N MCQs' from the actual allocation — no hardcoded values, no generic 'Full Mock NN'). Session/attempt titles match the exam-specific name. Mock count stays real (published mocks from mockSeries allocation; incomplete pools honestly show fewer). Admin BrainLab Manager gains Mock Package Status panel: per-exam pool size, mocks published N/10, questions per mock, cross-mock overlap (0 by construction), complete/incomplete marking. Zero engine/checkout/auth/payment changes — timer, palette, auto-submit, scoring, review, progress all untouched. // v115: exam cycle architecture (cycle ≠ PYQ source-year). // v114: EXAM UNIVERSE v3. // v115: EXAM CYCLE ARCHITECTURE — exam cycle ≠ PYQ source year, both data-driven. New ExamCycle backend (Base44 entity + euLive public read + euManage cycleSave/cycleDelete/cycleList admin ops) powers ADRE 1.0 · 2022, ADRE 2.0 · 2024, ADRE 3.0 · Upcoming (year to be announced — never invented). ADR org page shows Exam Cycles section + NEW cycle route #brainlab/exams/cycle/<id> (real official papers grouped by grade from the existing ADRE papers config, honest cycle PYQ state, post-wise exams). Hub fixes per spec §12 order: Mocks → PYQ (cycle chips + honest 'source year (provenance — NOT exam cycles)' labels; APSC-sourced overlap PYQs never labelled as ADRE papers) → Previous Year Papers (REAL official paper records grouped by cycle — no more year rows from arbitrary PYQ source years) → Subjects → Mistakes → Saved → CA → Materials → Syllabus → Pattern → Recommendations. Admin BrainLab Manager gains 🏛️ Exam Cycles manager (add/edit/delete cycles without frontend changes). SDK bug fixed: entity list() with options returns [] — euLive/euManage now list() bare + filter in code; anon entity reads were silently empty so euLive reads via service role (public-safe rows only). brainlab-exams.js v20260910d / pages v20260910b / exam-import v20260910c. Zero engine/checkout/auth/payment changes. // v114: EXAM UNIVERSE v3 — organizations layer, exam variants, Base44 question backend. // v114: EXAM UNIVERSE v3 — Organizations layer: BrainLab → Exam Universe → Organizations → Exam → complete per-exam Preparation Hub. New org landing (#brainlab/exams, real per-org counts) + org page (#brainlab/exams/org/<id>, real per-exam counts: questions/PYQ/mocks/PDFs) + exam variants (ADRE Grade III Driver, ADRE Grade IV Class VIII — honestly practicing from base pools, no fabricated questions). Hub adds Previous Year Papers (year-grouped real PYQs), exam-scoped Mistake Book, Saved Questions (DOM-additive 🔖 buttons injected into any engine review, localStorage store, practice-saved flow), registry-driven Preparation Roadmap with real completion states. Imported-question storage moved to the Studyria Exam Universe BASE44 BACKEND (ExamQuestion entity + euLive public read / euImport+euManage admin-gated server-side via Supabase session ↔ admin_users) — the bl_exam_questions migration is OBSOLETE and removed (never run). brainlab-exams.js v20260910c / pages v20260910a / exam-import v20260910b / css v20260910b. Zero engine/checkout/auth/payment changes; existing routes preserved. // v113: EXAM UNIVERSE v2 — 10-mock series, PYQ All/By-Year/By-Subject, daily exam practice, weak-area practice, syllabus coverage, admin import pipeline (Supabase bl_exam_questions — superseded by v114 backend). // v113: EXAM UNIVERSE v2 — complete per-exam preparation system. 10-mock series per exam (distinct non-overlapping sets from the REAL pool, deterministic seeded allocation, existing mock engine), PYQ practice All/By Year/By Subject, daily exam practice (date-seeded real questions, done-state), weak-area practice from own sessions, syllabus coverage from real bank topics, resource filter chips, admin bl_exam_questions import pipeline (Upload→Parse→Validate→Dedupe→Preview→Confirm→Report + verify manager; migration brainlab-exam-import-migration.sql PENDING user run) — only verified rows go live. ADRE Papers page linked. Zero engine/checkout/auth/DB changes. // v112: EXAM UNIVERSE — new additive brainlab-exams.js/.css. #brainlab/exams upgraded to Exam Universe landing (real stats + search) and NEW detail route #brainlab/exams/<id>: progress (own sessions), continue, real mock start, quick 10/25/50, PYQs, subject-wise, question papers + study materials via canonical openDetail PDF flow (checkout/ownership untouched), current affairs, real recommendations, about. Dashboard Exam Universe strip. Zero backend/DB/auth/payment changes. // v111: BrainLab final UX — Why Studyria BrainLab section (10 real-feature points), feature quick-access strip moved directly below hero, home flow per spec Part 5 (Hero → Strip → Stats → personal sections → module directory → Why), NEW Subject-wise Practice page #brainlab/subjects with real QB counts (topics/MCQs/PYQs per category), search, subject detail #brainlab/subjects/<slug> with real topic list + real user progress (own sessions only) + Start Full Subject/Topic Practice via existing count picker & learn player. brainlab-pages.js v20260908d / css v20260908c. No backend/checkout/payment/auth/DB changes. // v110: BrainLab Phase B — Mock Test Engine (brainlab-mock.js/.css, additive layer): exam-style mock player for mode==='mock' ONLY — no instant feedback, change/clear answers, question palette with answered/marked states, mark-for-review, countdown timer (1 min/question) with auto-submit at 00:00, submit summary; quiz/arena/daily keep the original learn player untouched; scoring/session/streak/mistakes still via the ORIGINAL _finishQuiz. No backend/checkout/payment/auth change. // v109: BrainLab Home IA order — Quick Access (Arena banner + Learning Modules directory) now sits directly under Hero/Stats, personal sections (Continue/Daily/Recommended/Streak/Tools) follow it; brainlab-pages.js -> v20260908c. Spec §7 flow. No backend/checkout/payment/auth change. // v108: BrainLab Home dashboard — prominent Practice Arena hero banner (real SA modes: Quick 10/25/50, Custom, Topic, Difficulty, Random — tappable, start the real player) + ENTER ARENA route; brainlab-pages.js/.css -> v20260908b. No backend/checkout/payment/auth change. // v107: BrainLab V8 modular pages (Dashboard + 11 dedicated module pages, brainlab-pages.js/.css new files) — bump wipes stale SWR HTML/JS so devices that had the site open before this deploy actually fetch the new index.html + new brainlab-pages bundle instead of running mismatched old cached code on the BrainLab nav item. No backend/checkout/payment/auth change. // v106: dedicated My Library page — #my-library is the ONE canonical personal-library route (openMyLibrary() → renderMyLibraryPage() → single-source BSF panel in #myLibMain); burger/header/Downloads 'My Library', PDF owned-state 'Go to My Library', free + paid payment success 'Open My Library', Pass page and Pass-Content CTAs all route there instead of the public catalog (#library) or the dashboard detour; Premium Library Universe sub-tab now always visible with a polished locked/upgrade state for non-members; legacy #premium-library hash deep-links to the Premium Universe tab; guest gate with sign-in + Explore button; empty state copy updated; cart.js→v20260907b, pco.js→v20260907d. Public/universal Library untouched. // v105: ONE canonical checkout routing — every PDF card click (paid/FREE/owned/premium, home/library/search/wishlist/featured/most-read/recently-added) now opens PCO directly via openDetail; card-level free/owned/price heuristics removed (they were the bug sending free + own PDFs to the old PDP checkout); free card CTAs route through the checkout FREE ACCESS experience (₹0/FREE → Get Free Access → grantFreeOwnership → premium success screen with Open in Library / Open PDF); owned card buttons keep intended direct open; handleBuy routes paid to PCO with buyPDF only as last-resort fallback. Asset versions v20260907c. // v104: premium checkout upgrade — dedicated FAILED/CANCELLED/FREE payment states, premium success screen (animated check, product card, payment ID, verified/library/lifetime checklist, Open Library + Read/Download CTAs), mobile swipe preview (stage-isolated passive pointer events, vertical scroll & pinch zoom untouched), refresh-safe checkout via #pdf-checkout/<id> hash + pco_last fallback, smart Back (returns to origin grid, never dead-ends on PDP), You-save line, focus-visible + aria-live/a11y, free-PDFs never open Razorpay, unused key constant removed; asset versions v20260907b. // v103: direct-to-checkout — every paid PDF card click (Library/Home/Search/OTT) now opens the premium PCO checkout directly, bypassing the PDP for the purchase flow; PDP retained for free + owned items; payment logic unchanged. Bump wipes stale SWR HTML/JS. // v102: single canonical checkout — old cart #checkout page RETIRED (page div, Cart.renderCheckout/_checkoutHTML/_bindCheckoutBtn removed); cart CTA routes single chargeable item to premium PCO checkout, multi-item carts batch-pay directly from the cart page; legacy #checkout URLs redirect to cart; cart.js/pco.js/cart.css bumped to v=20260907. Bump wipes stale SWR entries so devices fetch fresh JS. // v101: route-lazy architecture — 25 heavy route scripts (question-bank 2.4MB, ADRE papers, BrainLab/Arena, Campus, admin tools) + 4 route CSS now load on-demand via navigate() preload; creator-program.js extracted from index.html; studyria-notifications.js BrainLab deep-links now use __blReady retry. Bump wipes stale SWR entries so devices fetch fresh JS. // v100: standalone _ownedPdfIds init — _isOwned threw 'undefined has' killing static-page hydration; assets -> v20260906e; // v99: pdp-checkout.js ReferenceError fix (_pdfFallbackCache) — static /pdf/ pages were stuck on skeletons; bump wipes v98 entries so stale devices fetch v20260906d. // v98: checkout rebuild — pco.js/pco.css added, asset versions v=20260906*, static /pdf/ pages bumped to v=20260906c. Bump wipes v97 SWR entries that kept serving the OLD pdp-checkout.js (old checkout page) to devices.
